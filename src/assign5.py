...
#<include file="/opt/ros/indigo/share/turtlebot_navigation/launch/amcl_demo.launch">
    #<arg name="map_file" value="/home/kyuhas/catkin_ws/src/YuhasK/src/params/map.yaml" />
  #</include>
...
class voice_cmd_vel():
	...
	def setup_goals(self):
		...
		goal1 = PoseStamped(position=Pose(1,1,1))
		goal2 = PoseStamped(position=Pose(1,1,1))
		goal3 = PoseStamped(position=Pose(1,1,1))
		goal4 = PoseStamped(position=Pose(1,1,1))
		goals = []
		goals[0] = goal1
		goals[1] = goal2
		goals[2] = goal3
	...
	def moveForwardOneMeter(self):
		goal = PoseStamped()
		goal.position.x = self.robotPose.x + 1
		goal.header = Header(frame_id = 'map', stamp = rospy.get_rostime())
		self.moveBasePub.publish(goal)
		self.waitUntilGoalReached(goal)
	...
	def speechCb(self, msg):
		if msg.data.find("stop") > -1:
			...
			# move robot backward
			self.sendCmdVel(-0.5, 0)
		elif msg.data.find("move forward 1 meter") > -1:
